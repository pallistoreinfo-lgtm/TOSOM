...
import { siteSettings, type NavItem } from "@/config/site";
...
function normalizedNav(settings: SiteSettings) {
  return (settings.mainNav as NavItem[]).filter((item) => item.label !== "Home").map((item) => item.label === "Expert In" ? { ...item, label: "Expert Info" } : item);
}
...
export function Header({ settings = siteSettings }: { settings?: SiteSettings }) {
  const navItems = normalizedNav(settings);
  const { site, headerActions } = settings;
  return (
    <header className="relative z-50 w-full border-b border-[#dce9ef] bg-white"><div className="home-container flex h-[88px] items-center justify-between gap-5"><Link href="/" aria-label={site.name} className="shrink-0"><Logo className="h-14 w-auto" /></Link><DesktopNav navItems={navItems} /><div className="ml-auto flex items-center gap-3"><button aria-label="Search" className="hidden h-10 w-10 items-center justify-center text-[#082a55] lg:flex"><Search className="h-5 w-5" /></button><Link href={headerActions.quiz.href} className="hidden items-center gap-2 rounded-full bg-[#07835e] px-5 py-3 text-[12px] font-semibold text-white shadow-sm transition hover:bg-[#096f53] lg:flex"><Send className="h-3.5 w-3.5" /> {headerActions.quiz.label}</Link><Link href={headerActions.consultation.href} className="hidden rounded-full bg-[#07528c] px-5 py-3 text-[12px] font-semibold text-white shadow-sm transition hover:bg-[#063f6c] lg:block">{headerActions.consultation.label}</Link><a href={headerActions.shop.href} target="_blank" rel="noopener" className="hidden rounded-full border border-[#0b477a] px-5 py-[11px] text-[12px] font-semibold text-[#0b477a] transition hover:bg-[#f1f8fb] 2xl:block">{headerActions.shop.label}</a><MobileNav navItems={navItems} settings={settings} /></div></div></header>
  );
}
