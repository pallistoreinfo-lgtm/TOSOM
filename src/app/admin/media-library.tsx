...
import { Clipboard, ImagePlus, LoaderCircle, RefreshCw, Trash2, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4" role="dialog" aria-modal="true" aria-label="Media library">
      <div className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3 border-b bg-slate-50 px-5 py-3">
          ...
          <span className="text-sm text-slate-500">{media.length} uploaded images</span>
        </div>
        <div className="grid min-h-[300px] flex-1 gap-4 overflow-auto p-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {media.map((item) => <article key={item.path} className="overflow-hidden rounded-xl border bg-white"><div className="aspect-video bg-slate-100"><img src={item.url} alt="" className="h-full w-full object-cover" /></div><div className="p-3"><p className="truncate text-xs font-semibold" title={item.path}>{item.path.split("/").at(-1)}</p><p className="mt-1 text-xs text-slate-400">{Math.ceil(item.size / 1024)} KB</p><div className="mt-3 flex gap-2"><button onClick={() => copy(item.url)} className="inline-flex flex-1 items-center justify-center gap-1 rounded-md border px-2 py-1.5 text-xs font-semibold"><Clipboard className="h-3.5 w-3.5" />Copy URL</button><button onClick={() => remove(item)} aria-label={`Delete ${item.path}`} className="rounded-md border border-red-200 px-2 text-red-700"><Trash2 className="h-3.5 w-3.5" /></button></div></div></article>)}
          {!busy && media.length === 0 && <div className="col-span-full flex items-center justify-center text-sm text-slate-500">No images have been uploaded through the admin panel yet.</div>}
        </div>
