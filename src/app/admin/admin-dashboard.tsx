...
type Collection = "home" | "pages" | "blog" | "podcast" | "conditions" | "labtests";
...
export function AdminDashboard() {
  ...
  async function remove() {
    if (selected === "content/home.json") return;
  ...
  async function uploadImage(file?: File) {
    ...
    try {
      ...
      setStatus(`Image uploaded. Its URL was copied: ${url}`);
    } catch (caught) {
  ...
  const previewUrl = selected === "content/home.json" ? "/" : selected.startsWith("content/podcast/") ? `/podcasts/${slug}/` : `/${slug}/`;
      ...
      <div className="border-b bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-5 py-4">
          ...
          <div className="flex gap-2">
            ...
            <label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border bg-white px-4 text-sm font-semibold"><ImagePlus className="h-4 w-4" />Upload image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="hidden" onChange={(event) => { void uploadImage(event.target.files?.[0]); event.target.value = ""; }} /></label>
        ...
        <section className="min-w-0 rounded-xl border bg-white shadow-sm">
          ...
          {!selected ? <div className="flex min-h-[520px] items-center justify-center p-8 text-center text-slate-500"><div><FilePlus2 className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 font-semibold">Choose content from the left</p><p className="mt-1 text-sm">or create a new page, article, podcast, condition, or lab test.</p></div></div> : <>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
              ...
              <div className="flex flex-wrap gap-2"><a href={previewUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-2 rounded-lg border px-3 text-sm font-semibold"><ExternalLink className="h-4 w-4" />Preview</a>{selected !== "content/home.json" && <button onClick={remove} disabled={busy} className="inline-flex h-10 items-center gap-2 rounded-lg border border-red-200 px-3 text-sm font-semibold text-red-700 disabled:opacity-50"><Trash2 className="h-4 w-4" />Delete</button>}<button onClick={save} disabled={busy || !dirty} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#07835e] px-4 text-sm font-semibold text-white disabled:opacity-50">{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save changes</button></div>
            </div>
            <div className="border-b bg-amber-50 px-5 py-3 text-xs leading-5 text-amber-900"><strong>Editor format:</strong> {selected === "content/home.json" ? "Edit text inside quotation marks, keeping the JSON punctuation intact." : <>Details between the two <code>---</code> lines control the title, URL, SEO, image, and draft status. The text below them is the page body using Markdown.</>}</div>
            <textarea aria-label="Content editor" spellCheck className="min-h-[650px] w-full resize-y bg-[#fbfcfd] p-5 font-mono text-[14px] leading-6 outline-none" value={content} onChange={(event) => setContent(event.target.value)} />
          </>}
        </section>
      </div>
